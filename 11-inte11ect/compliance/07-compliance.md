...
impl ArabicLocalizer {
    pub fn ai_disclosure(&self) -> String {
        "هذه المحادثة مدعومة من Inte11ect AI. أنت تتفاعل مع نظام ذكاء اصطناعي. \
         لمزيد من المعلومات، يرجى زيارة https://inte11ect.ai/ai-disclosure".to_string()
    }

    pub fn risk_level(&self, level: &UaeRiskLevel) -> String {
        match level {
            UaeRiskLevel::Low => "منخفض",
            UaeRiskLevel::Medium => "متوسط",
            UaeRiskLevel::High => "عالٍ",
            UaeRiskLevel::Critical => "خطير",
        }.to_string()
    }

    pub fn ethical_principles(&self) -> Vec<String> {
        vec![
            "العدالة - Fairness".to_string(),
            "المساءلة - Accountability".to_string(),
            "الشفافية - Transparency".to_string(),
            "السلامة والأمان - Safety & Security".to_string(),
            "الخصوصية - Privacy".to_string(),
            "الرقابة البشرية - Human Control".to_string(),
            "الشمولية - Inclusivity".to_string(),
        ]
    }
}
```
