...
# feature-paper - Document 01

> **Associated Module:** Tec-11
## North Star Metric

The Tec-11 module defines and tracks the Inte11ect North Star Metric: **Local Inference Queries (LIQ)**. This metric measures the total number of AI inference queries completed locally on user hardware, serving as the single most important indicator of platform success and mission alignment.

### Why Local Inference Queries?

```mermaid
flowchart TD
    A[North Star: LIQ] --> B[User Value]
    A --> C[Privacy]
    A --> D[Sustainability]
    A --> E[Adoption]
    A --> F[Revenue]
    B --> B1[Fast responses]
    B --> B2[No cloud dependency]
    B --> B3[Offline capability]
    C --> C1[Data never leaves device]
    C --> C2[Zero server-side logs]
    D --> D1[98% less energy vs cloud]
    D --> D2[No data center needed]
    E --> E1[Free tier: unlimited queries]
    E --> E2[Viral: share local models]
    F --> F1[Enterprise seats]
    F --> F2[Premium model access]
```
...
{
    "north_star_metric": {
        "name": "Local Inference Queries",
        "abbreviation": "LIQ",
        "definition": "Total number of AI inference queries completed locally on user hardware within a given time period",
        "unit": "queries",
        "frequency": "daily, weekly, monthly, quarterly, annual",
        
        "inclusions": [
            "All model inference queries (text, code, image)",
            "Batch inference queries (each item counts)",
            "Speculative decoding queries (both draft and verify)",
            "API-compatible queries via compatibility layer"
        ],
        
        "exclusions": [
            "Model loading operations",
            "Indexing and embedding operations",
            "Failed or error queries",
            "Cloud fallback queries (tracked separately)"
        ],
...
impl NorthStarTracker {
    pub fn record_query(&self, query: &InferenceQuery) {
        // Increment the global counter
        let total = self.counter.fetch_add(1, Ordering::Relaxed) + 1;
        
        // Segment the query for later analysis
        let segment = self.segmenter.segment(query);
        
        // Add to daily aggregation
        self.daily_aggregator.add(segment);
        
        // Emit event for real-time dashboards
        self.event_emitter.emit(Event::InferenceQuery {
            user_id: query.user_id.clone(),
            model: query.model_id.clone(),
            hardware: query.hardware.clone(),
            timestamp: Utc::now(),
        });
        
        // Check if we hit a milestone
        if total % 1_000_000 == 0 {
            self.event_emitter.emit(Event::Milestone {
                metric: "LIQ".to_string(),
                value: total,
                milestone: format!("{}M", total / 1_000_000),
            });
        }
    }
    
    pub fn get_liq(&self, period: &TimePeriod) -> u64 {
        self.daily_aggregator.query(period)
    }
    
    pub fn get_liq_by_model(&self, model_id: &str, period: &TimePeriod) -> u64 {
        self.daily_aggregator.query_with_filter(
            period,
...
| Metric | Relationship to LIQ | Leading/Lagging | Target |
|--------|-------------------|-----------------|--------|
| Daily Active Users | LIQ = DAU Ã— queries_per_user | Leading | 500K by Q4 2026 |
| Queries per User per Day | Core driver of LIQ | Leading | 100 avg by Q4 2026 |
| Model Load Success Rate | Enables LIQ | Leading | >99.5% |
| Average Response Time | Affects user satisfaction | Lagging | <500ms P95 |
| User Retention (D30) | Long-term LIQ driver | Lagging | >60% |
| Viral Coefficient | New user acquisition | Leading | >1.5 |
| Net Promoter Score | User satisfaction | Lagging | >50 |
| Carbon per Query | Sustainability metric | Lagging | <0.1 gCO2e |

### LIQ Growth Model

```python
def project_liq_growth(
    current_dau: int,
    growth_rate: float,
    virality: float,
    ...
    """Project LIQ growth using compound model."""
    projections = []
    dau = current_dau
    
    for month in range(months):
        new_users = dau * growth_rate
        viral_users = dau * virality * 0.1  # 10% of users refer others
        churned_users = dau * 0.05  # 5% monthly churn
        
        dau = dau + new_users + viral_users - churned_users
        liq = dau * queries_per_user * 30  # Monthly LIQ
...
# Current projections
growth = project_liq_growth(
    current_dau=50000,
    growth_rate=0.08,    # 8% organic monthly growth
    virality=1.5,        # Each user refers 1.5 others
    queries_per_user=75,  # Average queries per user per day
    months=12,
)
```
...
```sql
-- North Star Dashboard Query
SELECT 
    DATE_TRUNC('day', timestamp) AS day,
    COUNT(*) AS liq,
    COUNT(DISTINCT user_id) AS dau,
    COUNT(*) / NULLIF(COUNT(DISTINCT user_id), 0) AS queries_per_user,
    COUNT(*) FILTER (WHERE model_id = 'qwen2-vl-2b-instruct') AS liq_2b,
    COUNT(*) FILTER (WHERE model_id = 'qwen2-vl-7b-instruct') AS liq_7b,
    COUNT(*) FILTER (WHERE hardware_type = 'gpu') AS liq_gpu,
    COUNT(*) FILTER (WHERE hardware_type = 'cpu') AS liq_cpu,
...
### North Star Review Cadence

```yaml
north_star_reviews:
  daily:
    participants: ["Engineering", "Product"]
    format: "Automated dashboard + Slack digest"
    key_questions:
      - "Is LIQ on track for weekly target?"
      - "Any anomalies in supporting metrics?"
      
  weekly:
    participants: ["Engineering", "Product", "Design"]
    format: "30-minute standup"
    key_questions:
      - "What drove LIQ this week?"
      - "What risks could impact LIQ next week?"
      - "Are there any user-reported issues affecting LIQ?"
      
  monthly:
    participants: ["All teams"]
    format: "1-hour business review"
    key_questions:
      - "Are we on track for quarterly LIQ target?"
      - "Which segments are over/underperforming?"
      - "What experiments will move LIQ next month?"
      
  quarterly:
    participants: ["Leadership", "Board"]
    format: "2-hour strategy offsite"
    key_questions:
      - "Did we hit LIQ targets? Why or why not?"
      - "What are the updated projections?"
      - "Do we need to adjust strategy?"
```

### LIQ vs Alternative Metrics

| Metric | Why Not North Star | Inte11ect's Choice |
|--------|-------------------|-------------------|
| Revenue | Chases short-term profits over mission | LIQ aligns with mission first |
| Active Users | Users could use cloud alternatives | LIQ specifically measures local |
| Model Downloads | One-time event, not ongoing value | LIQ measures sustained engagement |
| User Satisfaction | Lagging indicator, hard to measure | LIQ is real-time and objective |
| Market Share | Hard to measure accurately | LIQ is precise and trackable |
...
Local Inference Queries (LIQ) is Inte11ect's North Star Metric because it directly measures the core value proposition: delivering AI inference that is private, local, efficient, and accessible. Every other metric feeds into LIQ, and every product decision is evaluated against its impact on LIQ growth.
