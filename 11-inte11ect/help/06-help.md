...
The Art-11 module documents issues with the .aioss ledger and audit system.

### Common Ledger Issues

**Issue**: Audit log grows too large.
**Solution**: Configure retention: `inte11ect-cli config set audit.retention_days 90`

**Issue**: Signature verification fails.
**Solution**: Update verification keys: `inte11ect-cli security update-keys`

**Issue**: Missing audit events.
**Solution**: Check logging level: `inte11ect-cli config set logging.level info`

### Ledger Management

```bash
inte11ect-cli audit status             # Check audit system status
inte11ect-cli audit export             # Export audit logs
inte11ect-cli audit verify            # Verify audit log integrity
inte11ect-cli audit search --user X   # Search audit logs by user
```
...
The audit ledger system is self-healing for most issues. For persistent problems, contact enterprise support.
