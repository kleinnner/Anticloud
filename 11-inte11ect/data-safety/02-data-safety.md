...
# data-safety - Document 02

> **Associated Module:** Game-11
## Encryption Standards

The Game-11 module defines the encryption standards used throughout Inte11ect to protect data at rest, in transit, and during processing.
...
flowchart TD
    ...
    B --> B1[AES-256-GCM]
    B --> B2[ChaCha20-Poly1305]
    B --> B3[OS Keychain/DPAPI]
    
    C --> C1[TLS 1.3]
    C --> C2[mTLS]
    C --> C3[Certificate Pinning]
    
    D --> D1[Secure Memory Allocation]
    D --> D2[mlock]
    D --> D3[Guard Pages]
```
...
|----------|-----------|----------|------|-----------|
| Local file encryption | AES-256-GCM | 256 bits | Authenticated encryption | Hardware-accelerated on modern CPUs |
| Local file (alt) | ChaCha20-Poly1305 | 256 bits | AEAD | Mobile devices without AES hardware |
| Model weights | AES-256-GCM | 256 bits | Authenticated | Large file encryption |
| Network (TLS) | TLS_AES_256_GCM_SHA384 | 256 bits | AEAD | TLS 1.3 mandatory cipher |
| Network (mTLS) | TLS_CHACHA20_POLY1305_SHA256 | 256 bits | AEAD | Alternative TLS 1.3 cipher |
| .aioss signing | Ed25519 | 256 bits | Asymmetric | Fast, secure signature scheme |
| Key derivation | HKDF-SHA256 | 256 bits | KDF | NIST-approved KDF |
| Password hashing | Argon2id | Configurable | Memory-hard | Password storage |
| Random generation | ChaCha20 DRBG | 256 bits | CSPRNG | Fast, secure RNG |

### Key Management

Keys are managed through the operating system's secure storage: Windows DPAPI for Windows, Apple Keychain for macOS, Linux Secret Service/libsecret for Linux, and TPM-backed storage where available.

### Data-in-Use Protection

Sensitive data in memory is protected through mlock (prevents swapping), guard pages, and automatic zeroing after use.
...
Inte11ect implements industry-standard encryption throughout the stack, with hardware-accelerated algorithms and OS-level key management ensuring strong protection with minimal performance impact.
