...
# data-safety - Document 01

> **Associated Module:** Code-11
## .aioss Specification

The Code-11 module defines the .aioss (AI Open Signed Storage) format specification â€” a file format designed for verifiable, signed, and portable AI artifacts. This document provides the complete specification.

### Format Overview

.aioss is a structured data format for AI artifacts that provides built-in verification, signing, and audit capabilities. Every .aioss file contains cryptographically signed metadata ensuring authenticity and integrity.

### File Structure

.aioss files use the `.aioss` extension and follow a JSON-based structure with mandatory cryptographic signing:

```
.aioss File Structure:
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚ Magic Bytes: "AIOSS"       â”‚  â† 4 bytes file type identifier
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Version: "1.0.0"           â”‚  â† Format version
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Type: "model_manifest"      â”‚  â† Artifact type
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Payload: { ... }            â”‚  â† The actual content
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Signature: { ... }          â”‚  â† Ed25519 signature
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Verification: { ... }       â”‚  â† Verification metadata
...
{
    "aioss_version": "1.0.0",
    "type": "model_manifest | inference_record | audit_entry | config | report",
    "created_at": "ISO8601 timestamp",
    "payload": {},
    "signature": {
        "algorithm": "Ed25519",
        "public_key_fingerprint": "string",
        "signature_value": "hex_encoded_signature",
...
|------|-------------|-----------------|
| model_manifest | Model metadata and integrity info | Model ID, version, hash, quantization |
| inference_record | Single inference operation | Prompt, response, timing, energy |
| audit_entry | Security or audit event | Event type, user, action, resource |
| config | User or system configuration | Configuration key-value pairs |
| report | Environment or usage report | Aggregated metrics and statistics |

### Signing Process

```rust
pub fn sign_aioss_file(payload: &serde_json::Value, private_key: &Ed25519Key) -> AiossFile {
    let payload_bytes = serde_json::to_vec(payload).unwrap();
    let hash = Sha256::digest(&payload_bytes);
    let signature = private_key.sign(&hash);
    
    AiossFile {
        aioss_version: "1.0.0".to_string(),
        type_: "model_manifest".to_string(),
        created_at: Utc::now(),
        payload: payload.clone(),
        signature: AiossSignature {
            algorithm: "Ed25519".to_string(),
            public_key_fingerprint: private_key.fingerprint(),
            signature_value: hex::encode(signature.to_bytes()),
            timestamp: Utc::now(),
        },
        verification: AiossVerification {
...
```rust
pub fn verify_aioss_file(file: &AiossFile, public_key: &Ed25519PublicKey) -> VerificationResult {
    let payload_bytes = serde_json::to_vec(&file.payload).unwrap();
    let hash = Sha256::digest(&payload_bytes);
    let signature_bytes = hex::decode(&file.signature.signature_value).unwrap();
...
The .aioss extension is registered with IANA and recognized by major operating systems for association with the Inte11ect platform.

### MIME Type

`application/vnd.inte11ect.aioss+json`

### Version History

| Version | Date | Changes |
|---------|------|---------|
| 1.0.0 | 2026-06-01 | Initial specification |
...
The .aioss format provides a secure, verifiable, and portable standard for AI artifacts. Built-in cryptographic signing ensures authenticity and integrity, while the extensible JSON-based format supports any type of AI data.
