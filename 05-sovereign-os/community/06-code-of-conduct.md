...
```mermaid
graph TD
    R[Report Received] --> A{Assignee}
    A --> Investigate[Investigate within 48h]
    Investigate --> Determine{Determine Severity}
    Determine -->|Minor| Correction[Correction]
    Determine -->|Moderate| Warning[Warning]
    Determine -->|Serious| TempBan[Temporary Ban]
    Determine -->|Severe| PermBan[Permanent Ban]
    Correction --> Communicate[Communicate Decision]
    Warning --> Communicate
    TempBan --> Communicate
    PermBan --> Communicate
    Communicate --> Record[Record in Confidential Log]
    Communicate --> Appeal{Appeal?}
    Appeal -->|Yes| Review[Review by different maintainer]
    Review --> FinalDecision[Final Decision]
    Appeal -->|No| Closed[Case Closed]
```
