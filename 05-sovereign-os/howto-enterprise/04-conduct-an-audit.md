...
This guide covers using the 01s-ledger for compliance audits, including generating audit reports, verifying integrity, and exporting evidence.

## Audit Overview

```mermaid
graph TD
    A[Start Audit] --> B[Export Ledger Data]
    B --> C[Verify Hash Chain]
    C --> D{Chain Intact?}
    D -->|Yes| E[Generate Audit Report]
    D -->|No| F[Investigate Tampering]
    F --> G[Identify Compromised Entries]
    G --> H[Document Incident]
    E --> I[Sign State Proof]
...
```bash
# Ensure ledger is initialized and has entries
01s-ledger status
...
## Step 2: Export Ledger Data

```bash
# Export full ledger
01s-ledger export > /tmp/audit-export-$(date +%Y-%m-%d).json

# Export specific session
01s-ledger export "session_2026_06" > /tmp/audit-session-export.json

# Export all available data
OUTPUT_DIR="/var/lib/01s/audit/$(date +%Y-%m-%d)"
mkdir -p "$OUTPUT_DIR"
for ledger_file in ~/ledger/*.aioss; do
    session=$(basename "$ledger_file" .aioss)
    01s-ledger export "$session" > "$OUTPUT_DIR/${session}.json"
done
```

## Step 3: Generate State Proof

```bash
# Generate cryptographic state proof
01s-ledger sign > /tmp/state-proof.txt
...
# Entries:    42
# Verify with: 01s-ledger sign <key_hex>

# Save for audit
cp /tmp/state-proof.txt "$OUTPUT_DIR/state-proof-$(date +%Y%m%d).txt"
```

## Step 4: Verify Hash Chain Programmatically

```python
#!/usr/bin/env python3
"""Comprehensive ledger audit verification script."""
import json
import hashlib
import subprocess
...
class LedgerAuditor:
    ...
    def verify_chain(self):
        results = {
            'verified': True, 'total_entries': 0,
            'errors': [], 'chain_integrity': True
        }
        ...
        for i, entry in enumerate(entries):
            canonical = {k: v for k, v in sorted(entry.items()) if k != 'hash'}
            ...
            if computed != entry.get('hash', ''):
                ...
                results['errors'].append(f"Entry {i}: hash mismatch")
            
            if entry.get('parent_hash', '') != prev_hash:
                results['verified'] = False
                results['chain_integrity'] = False
                results['errors'].append(f"Entry {i}: parent chain broken")
            
            prev_hash = entry.get('hash', '')
        
        return results
    
    def generate_report(self, auditor_name="01s Audit"):
        results = self.verify_chain()
        report = f"""
===============================================================
   01s SOVEREIGN — AUDIT REPORT
===============================================================
...
Chain Integrity: {'INTACT' if results['chain_integrity'] else 'BROKEN'}
Overall:         {'VERIFIED' if results['verified'] else 'TAMPERED'}

ERRORS:
{chr(10).join(results['errors']) if results['errors'] else 'No errors detected.'}
===============================================================
"""
        return report

auditor = LedgerAuditor('/home/alice/ledger/2026-06-19.aioss')
print(auditor.generate_report('Compliance Team'))
```

## Step 5: Health Ledger Audit
...
echo "=== Health Ledger Audit ==="
echo "Date: $DATE"

if [ ! -f "$HEALTH_FILE" ]; then
    echo "No health ledger found for $DATE"
    exit 0
fi
...
# Summarize results
PASS=$(grep -c '"status":"pass"' "$HEALTH_FILE" || true)
FAIL=$(grep -c '"status":"fail"' "$HEALTH_FILE" || true)
WARN=$(grep -c '"status":"warn"' "$HEALTH_FILE" || true)

echo "Tests:    $((PASS + FAIL + WARN))"
echo "Passed:   $PASS"
echo "Failed:   $FAIL"
echo "Warnings: $WARN"

# Log audit
01s-ledger log audit_event type=health_audit date="$DATE" passed="$PASS" failed="$FAIL"
```
...
echo "=== 01s Compliance Report Generator ==="

# 1. Ledger status
01s-ledger status > "$OUTPUT_DIR/ledger-status.txt"

# 2. Chain verification
01s-ledger verify > "$OUTPUT_DIR/chain-verification.txt"

# 3. Toolchain integrity
...
# 6. Entry log export
01s-ledger export > "$OUTPUT_DIR/ledger-export.json"

# Create evidence package
tar czf "$OUTPUT_DIR/../compliance-$(date +%Y%m%d).tar.gz" -C "$(dirname $OUTPUT_DIR)" "$(basename $OUTPUT_DIR)"
echo "Compliance package created"
```

## Audit Checklist

- [ ] Ledger initialized and operational
- [ ] `01s-ledger verify` passes
- [ ] State proof generated and saved
- [ ] Health ledger verified
- [ ] Toolchain integrity confirmed
