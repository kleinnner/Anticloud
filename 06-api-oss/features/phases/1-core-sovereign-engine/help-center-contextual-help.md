---
title: "Help Center + Contextual Help"
sidebar_position: 99
description: "Provides 51 help articles accessible via a Help Center view. Contextual help displays relevant articles based on the current view — when you open a view, the help panel shows articles specific to that"
tags: [features]
---
...
## What It Does
Provides 51 help articles accessible via a Help Center view. Contextual help displays relevant articles based on the current view — when you open a view, the help panel shows articles specific to that feature. A floating button and quick panel components provide access from anywhere. All articles are in Markdown and render locally with full offline availability.

## How It Works
The help system combines React 18 frontend components with a lightweight backend. 51 help articles stored as Markdown files in `./data/help/`. On startup, the system loads the article index and makes it available via WebSocket on port 3030.

Frontend components: HelpCenterView (full browser), HelpFloatingButton ("?" button, bottom-right), HelpQuickPanel (slide-out contextual panel, 3-5 articles), HelpArticleViewer (Markdown renderer with syntax highlighting).

Contextual awareness: frontend detects the current route and matches against article tags defined in each article's frontmatter. For example, the Graph view shows articles about "Canvas Graph Rendering" and "Node Types". No contextual match falls back to most popular articles.

The system runs on the single `api-oss` binary. HTTP UI on port 8081. CLI has 87 commands across 9 subcommand groups. Everything works fully offline.
...
3. Click the "?" floating button in the bottom-right corner of any view.
4. The HelpQuickPanel slides out showing contextual articles for the current view.
5. Click an article to open it in the full HelpCenter view.
6. Browse all 51 articles by category (Getting Started, Graph, Chat, Codex, etc.).
7. Search articles by keyword using the search bar.
8. CLI: `api-oss help list`, `api-oss help article --slug "graph-rendering"`.
9. Add custom articles by placing Markdown files in `./data/help/`.

Config:
```json
{
  "help": {
    "articles_dir": "./data/help",
    "contextual_matching": true,
    "max_quick_panel_articles": 5
  }
}
```

## The Moat
- Most AI systems have no built-in help — users visit external docs websites.
- Our help is embedded, view-aware, and fully offline.
- 51 pre-bundled articles cover every feature comprehensively.
- Users add custom articles without recompiling — just drop in Markdown files.
- Floating button provides always-accessible help without leaving the current task.

## Why Choose API-OSS
Built-in help reduces support costs, improves onboarding, and works offline. Enterprise deployments add custom documentation for internal workflows. Critical for air-gapped deployments where external docs are inaccessible.

## Competitive Comparison
- **Palantir**: External documentation — no in-app contextual help.
- **OpenAI**: External docs and community forums — no embedded help in ChatGPT.
- **Anthropic**: External documentation only.

## Cost-Benefit Analysis
OpenAI charges $0.15/1M input and $0.60/1M output tokens. Cloud knowledge bases (Zendesk Guide $19-$149/month, Confluence Cloud) cost monthly. API-OSS provides 51 articles with contextual delivery at zero cost. Time savings: in-app answers vs. external doc searches. Risk reduction: help available offline.

## Applications
- **Consumer**: Get help without leaving the app or requiring internet.
- **Government / Defense**: Offline-accessible docs for classified deployments.
- **Enterprise**: Reduces support tickets with context-aware help.

## Protocol Message Reference
Request: `{"type":"search_help","query":"graph rendering"}`. Response: `{"type":"help_search_results","results":[{"slug":"graph-rendering","title":"Canvas Graph Rendering","snippet":"..."}]}`. Request: `{"type":"get_help_article","slug":"graph-rendering"}`. Response: `{"type":"help_article","title":"...","content":"# Markdown..."}`.

## CLI Command Reference
`api-oss help list`, `api-oss help list --category graph`, `api-oss help article --slug "graph-rendering"`, `api-oss help search "chat"`, `api-oss help refresh`.

## Security Considerations
Help articles stored locally in `./data/help/`. Local Markdown renderer — no external CDN or network requests. Custom articles sandboxed — dangerous HTML tags stripped.
