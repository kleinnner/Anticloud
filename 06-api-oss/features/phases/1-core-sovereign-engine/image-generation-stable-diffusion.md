...
sidebar_position: 99
description: "Runs Stable Diffusion 1.5 GGUF models locally with no API key, no rate limits, and no content filters. Generates images directly on-device using CUDA on the 3050 Ti GPU. The model is downloaded once a"
tags: [features]
---
...
## What It Does
Runs Stable Diffusion 1.5 GGUF models locally with no API key, no rate limits, and no content filters. Generates images directly on-device using CUDA on the 3050 Ti GPU. The model is downloaded once and then operates fully offline. Users control generation parameters: prompt, steps, CFG scale, seed, and output size. Images are saved locally with full prompt metadata.

## How It Works
The image generation engine lives in i-oss-gateway/src/sd.rs. It loads Stable Diffusion 1.5 in GGUF format � a quantized format that reduces model size from ~2GB to ~500MB while maintaining output quality. The GGUF model is loaded into GPU memory via llama.cpp's SD backend bindings.

Generation pipeline: user provides a text prompt via WebSocket on port 3030 (generate_image message). The prompt is encoded by the CLIP text encoder. The UNet denoising process runs for the specified number of steps (default 20) on the 3050 Ti GPU with CUDA backend. The VAE decoder converts the latent representation to a pixel image. The image is saved to ./data/images/ with a unique filename and metadata (prompt, seed, parameters). The image URL and thumbnail are returned via WebSocket.

Parameters: steps (1-100), CFG scale (1-20), seed (for reproducible generation), width/height (512x512 default, up to 768x768 on a 3050 Ti with 4GB VRAM). A negative prompt can be specified to exclude concepts.

The frontend ImageGenerationView (React 18 + Vite 5 + Tailwind) provides a prompt input field, parameter sliders, seed randomizer, and image gallery. The system runs on the single pi-oss binary. HTTP UI on port 8081. CLI has 87 commands across 9 subcommand groups including image generate --prompt "cat" --steps 30.
...
3. Navigate to the Image Generation view.
4. Enter a prompt (e.g., "a serene mountain lake at sunset, digital art style").
5. Optionally enter a negative prompt (e.g., "blurry, low quality, distorted").
6. Adjust steps (20-50 for quality), CFG scale (7-12 typical), seed (0 for random).
7. Click "Generate". Progress is shown in real time. The generated image appears in the gallery.
8. Use CLI: pi-oss image generate --prompt "cat" --steps 30 --seed 42.
9. Use pi-oss image list to view all generated images, pi-oss image delete --id <id>.
...
## The Moat
- OpenAI DALL-E is cloud-only, censored (content filters block certain prompts), and charges per image (.04-.08 per image).
- Our image generation is free per-image, uncensored, and works offline in air-gapped environments.
- The GGUF format allows model quantization for lower VRAM usage while maintaining quality.
- No rate limits � generate as many images as you want, as fast as your GPU can handle.
- All generation is local � prompts and images never leave the device.

## Why Choose API-OSS
Unlimited free image generation with complete privacy. Organizations that need to generate images for presentations, marketing, training materials, or classified work can do so without per-image costs, rate limits, or content policy restrictions. The local CUDA-accelerated pipeline generates images 2-5x faster than cloud API calls when network latency is considered.

## Competitive Comparison
- **OpenAI**: DALL-E 3 � cloud-only, censored content filters, pay-per-image pricing (.04-.08/image).
- **Anthropic**: No image generation capability in Claude.
- **Google**: Imagen � cloud-only, usage quotas, content policies, enterprise licensing.
- **Nvidia**: Picasso � cloud-based, enterprise pricing, no local deployment option.
