---
title: "Output Studio / Artifact Generation"
sidebar_position: 99
description: "Generates polished PDF documents from system decisions, reports, and graph content. Supports export of decision rationales, council deliberation summaries, contradiction reports, and custom content. A"
tags: [features]
---

# Output Studio / Artifact Generation

## What It Does
Generates polished PDF documents from system decisions, reports, and graph content. Supports export of decision rationales, council deliberation summaries, contradiction reports, and custom content. All exports are recorded in the SHA-256 hash-chained audit ledger. Each export is cryptographically linked to its source data, providing verifiable provenance.

## How It Works
The Output Studio lives in i-oss-gateway/src/handlers/output.rs and the frontend OutputStudioView (React 18 + Vite 5 + Tailwind). When a user requests an export via WebSocket on port 3030 (export_pdf message):

1. The system gathers the requested content from the knowledge graph at data/graph.db or the audit ledger at data/ledger/.
2. Content is rendered to PDF using a local PDF generation library � no external API, no cloud dependency.
3. Metadata is embedded in the PDF: export timestamp, source graph node IDs, ledger entry hash, and a SHA-256 fingerprint of the content.
4. The PDF is saved to ./data/exports/ and the file path is returned via WebSocket.
5. A ledger entry is created recording the export with its content hash, linking back to the source data.

Export types: "Decision Rationale" (full decision context with agent votes), "Council Summary" (deliberation records), "Contradiction Report" (detected contradictions with resolution status), "Graph Report" (selected nodes and edges in structured format), "Custom Content" (user-specified text rendered to PDF).

The frontend provides a preview pane showing the rendered document before export, a download manager for previous exports, and a delete option. The system runs on the single pi-oss binary. HTTP UI on port 8081.

## How to Operate
1. Launch the gateway: pi-oss start or run the binary directly.
2. Open http://localhost:8081 (React 18 + Vite 5 + Tailwind frontend).
3. Navigate to the Output Studio view.
4. Select the export type from the dropdown (Decision Rationale, Council Summary, Contradiction Report, etc.).
5. Select the source data (decision, session, graph nodes) to include.
6. Preview the document in the preview pane.
7. Click "Export" to generate the PDF. It is saved to ./data/exports/.
8. Previous exports are listed in the download manager � click to download or delete.
9. Use CLI: pi-oss export list, pi-oss export delete --id <id>.
10. Use CLI: pi-oss export pdf --type contradiction --id <contradiction-id>.
...
## The Moat
- OpenAI offers no export capability � chat data is locked in their cloud with no PDF generation.
- Our Output Studio produces professional PDF artifacts with cryptographic provenance links.
- Each export is linked to its source via ledger entry hash � tamper-proof provenance.
- No external API or cloud service is needed for PDF generation.
- Multiple export types cover the full range of system outputs.

## Why Choose API-OSS
Professional document generation from AI decisions with full audit trail. Organizations that need to produce reports, compliance documentation, or decision rationales can generate polished PDFs with cryptographic proof of origin. No competitor offers PDF export with integrated provenance tracking � data is trapped in their chat interfaces.

## Competitive Comparison
- **OpenAI**: No export functionality � data is trapped in chat history. Cannot generate PDFs from conversations.
- **Anthropic**: No PDF export � copy-paste only. No document generation capability.
- **Google**: Gemini has no structured document export. No PDF generation from conversations.
