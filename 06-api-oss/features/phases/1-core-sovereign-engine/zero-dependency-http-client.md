---
title: "Zero-Dependency HTTP Client"
sidebar_position: 99
description: "A custom HTTP client built on raw TcpStream with no external dependencies like eqwest or hyper. Handles DNS resolution, TLS, redirects, and connection pooling internally. Used primarily for DuckDuckG"
tags: [features]
---
...
## What It Does
A custom HTTP client built on raw TcpStream with no external dependencies like eqwest or hyper. Handles DNS resolution, TLS, redirects, and connection pooling internally. Used primarily for DuckDuckGo Lite HTML scrape for web search � no API key required. The entire client is a single Rust module with zero crate dependencies for HTTP.

## How It Works
The zero-dependency HTTP client lives in i-oss-gateway/src/http_client.rs. It is a hand-written HTTP/1.1 client implementation with the following capabilities:

**DNS resolution**: Uses the OS's DNS resolver via std::net::ToSocketAddrs. No external DNS library required. Caches DNS results with a configurable TTL (default 300 seconds).

**TCP connection**: Opens a raw TcpStream to the resolved IP address and port. Connection pooling reuses keep-alive connections within a configurable timeout (default 60 seconds).

**TLS**: Uses OS-native TLS via 
ative-tls crate (the only non-stdlib dependency, required for HTTPS). Certificate verification uses the system certificate store.

**HTTP/1.1**: Manual request serialization and response parsing. Handles: GET and POST methods, headers, chunked transfer encoding, content-length, connection keep-alive, and redirect following (up to 5 redirects by default).

**Connection pooling**: A simple pool of TcpStream connections keyed by host:port. Connections in the pool are checked for liveness before reuse. Idle connections are closed after the timeout.

The client is used by the web search module (handlers/web_search.rs) to scrape DuckDuckGo Lite HTML search results. The entire HTTP client is ~500 lines of Rust code with one TLS dependency � compared to eqwest which pulls in 200+ transitive crates.

The system runs on the single pi-oss binary. Everything works fully offline when web search is disabled. CLI has 87 commands across 9 subcommand groups.

## How to Operate
1. The zero-dependency HTTP client works automatically when web search is enabled.
2. Launch the gateway: pi-oss start or run the binary directly.
3. Web search uses the client internally � no configuration needed.
4. To verify HTTP client operation: pi-oss web search "test" from CLI.
5. The client is not user-configurable day-to-day, but behavior can be adjusted in config.
6. When search.enabled: false, the client is never initialized � zero network code runs.

Config in opencode.json:
`json
{
  "http_client": {
    "user_agent": "API-OSS/1.0",
    "timeout_secs": 10,
    "max_redirects": 5,
...
## The Moat
- Every competitor's HTTP client depends on the OS or language ecosystem's HTTP library (reqwest, hyper, curl, etc.), all pulling in hundreds of transitive dependencies.
- Our client is a single Rust module with only one non-stdlib dependency (native-tls for TLS).
- Minimizing attack surface: fewer dependencies = fewer potential vulnerabilities.
- Minimizing compile time: zero transitive crate dependencies for HTTP operations.
- Minimizing binary size: a few hundred lines of Rust vs. hundreds of compiled dependency crates.
- The client is purpose-built for the system's needs � no unused features, no bloat.

## Why Choose API-OSS
Supply-chain security through dependency minimization. Organizations with strict security requirements (defense, critical infrastructure) benefit from software with minimal dependencies. The zero-dependency HTTP client reduces the attack surface for web search operations to the absolute minimum � one Rust module and OS-native TLS.

## Competitive Comparison
- **OpenAI**: Requires Bing Search API key for web search � external dependency plus API key management.
- **Anthropic**: No web search capability built in. No HTTP client needed for search.
- **Google**: Uses own paid search API � requires Google Cloud SDK dependencies.
