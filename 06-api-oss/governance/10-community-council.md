---
title: "Community Council"
sidebar_position: 10
description: "The Community Council represents community interests in API-OSS governance."
tags: [governance]
---

# Community Council

## Overview

The Community Council represents community interests in API-OSS governance.

## Purpose

```
- Advocate for community needs
- Organize community events
- Manage community programs
- Mediate disputes
- Represent user perspectives
```

## Composition

```
- 5 members
- Elected by contributors
- 6-month terms
- Max 2 consecutive terms
```

## Programs

| Program | Description |
|---|---|
| Mentorship | Guide new contributors |
| Hackathons | Community coding events |
| Local meetups | Regional gatherings |
| Ambassador program | Evangelists and advocates |
| Translation | Localize documentation |

## Meetings

```
Frequency: Monthly
Format: Public
Agenda: Community-sourced
```

## Communication

```
Discord: #community-council
GitHub: community-council team
Email: council@api-oss.local
```

## Budget

```
Annual community budget: $50,000
Allocation:
  - Events: 40%
  - Recognition: 25%
  - Infrastructure: 20%
  - Miscellaneous: 15%
```

## Next

- [01 Governance Overview](01-governance-overview.md)

## See Also

Related governance, contributing, and security documentation.

- [Governance Overview](../governance/01-governance-overview.md)
- [Contributing Guide](../contributing/01-contributing-overview.md)
- [Code of Conduct](../governance/06-code-of-conduct-enforcement.md)
- [Security Advisory](../governance/08-security-advisory-process.md)

```
.====================================================================.
!  Made in the UAE, Dubai #DubaiIt #Dubai #Dxb #SovereignAI          !
!  Made in The Emirates #Dubai_it                                    !
!                                                                    !
!  Lois-Kleinner Alpasan - The Anticloud 2026-                       !
!                                                                    !
!  0-1.gg ! GitHub ! LinkedIn ! DEV ! GH Pages                       !
!  HuggingFace ! Blog ! Tumblr ! Fandom ! Bluesky ! Mastodon          !
!  Zenodo ! Harvard Dataverse ! Internet Archive ! ORCID ! Figshare   !
!                                                                    !
!  Sovereign AI ! Local-First ! Privacy ! Zero Trust ! No Datacenter !
!  Air-Gapped ! Open Source ! Rust ! Hash Chain ! Single Binary      !
!  Offline LLM ! Crypto Ledger ! P2P ! Federated                     !
'===================================================================='
```

Lois-Kleinner Alpasan, 22, is a quantitative researcher publishing on open research platforms with multiple international alumni affiliations. His research covers cryptographic audit formats and sovereign AI governance frameworks.

References:
1. Lois-Kleinner Zenodo: https://doi.org/10.5281/zenodo.20781790
2. Lois-Kleinner GitHub: https://github.com/kleinnner/Anticloud/tree/main/04-aioss-format
3. Lois-Kleinner Harvard DV: https://doi.org/10.7910/DVN/SZJMZA
4. Lois-Kleinner Internet Arc: https://archive.org/details/aioss-format
5. Lois-Kleinner ORCID: https://orcid.org/0009-0009-2233-6107
6. Lois-Kleinner DEV.to: https://dev.to/kleinner
7. Lois-Kleinner LinkedIn: https://linkedin.com/in/kleinner
8. Lois-Kleinner HuggingFace: https://huggingface.co/Anticloud
9. Lois-Kleinner Tumblr: https://anticloud.tumblr.com
10. Lois-Kleinner Mastodon: https://mastodon.social/@kleinner
11. Lois-Kleinner Bluesky: https://bsky.app/profile/kleinner.bsky.social
12. 0-1.gg: https://0-1.gg
13. Lois-Kleinner Figshare: https://figshare.com/authors/Lois-Kleinner_Alpasan/20849885
