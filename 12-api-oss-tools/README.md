...
flowchart TD
    ...
    subgraph Compliance["Compliance & Governance"]
        ...
        CG[Compliance Gap Analyzer]
        CP[Compliance Generator]
        DR[Data Residency Map]
        SC[SSP Generator]
        SB[Supply Chain SBOM]
        VR[Vendor Risk Score]
    end
    subgraph Analysis["Analysis & Planning"]
        AC[Architecture Canvas]
        CC2[Contract Clause Analyzer]
        DE[Deploy Simulator]
        DCE[Deployment Cost Estimator]
        IC[Integration Checker]
        RP[RFP Response]
        RC[ROI Calculator]
    end
    subgraph Tools["Developer Utilities"]
        DV[Diff Viewer]
        FT[Focus Timer]
        HT[Habit Tracker]
        JE[JSON Explorer]
        ...
        PM[Port Protocol Mapper]
        PP[Privacy Scanner]
        RQ[Readiness Quiz]
        RP2[Regex Playground]
        SF[SQL Formatter]
        TC[TCO Calculator]
    end
```
